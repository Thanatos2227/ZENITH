...
export class ScoringService {
  ...
  public calculatePriceImpact(params: {
    ...
    // 1. Authoritative DEX/Aggregator price impact takes priority if available
    ...
    // 2. Guard against non-positive amounts
    if (params.amountInNum <= 0 || params.amountOutExpectedNum <= 0) {
      return { percentage: 0, level: 'NEGLIGIBLE' };
    }

    // 3. Determine pre-trade reference market price (tokenOut per tokenIn)
    ...
    // If no reliable reference price is available, do not invent or use fake values (e.g. 0.05%)
    if (!refPrice || refPrice <= 0 || isNaN(refPrice) || !isFinite(refPrice)) {
      return { percentage: 0, level: 'NEGLIGIBLE' };
    }

    // 4. Calculate net amount swapped against the pool curve by excluding swap and platform fees
    ...
    // 5. Execution price on the pure liquidity curve (excluding fees)
    ...
    // 6. Realized price impact: max(0, (1 - executionPrice / referencePrice) * 100)
