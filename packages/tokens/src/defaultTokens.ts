...
/** The single built-in token registry. Keep network-specific addresses in this file. */
const NORMALIZED_TOKENS: Token[] = TOKEN_CONFIG.map((token) => {
  if (!token.isNative) {
    return { ...token, enabled: token.enabled !== false };
  }

  const wrappedToken = TOKEN_CONFIG.find(
    (candidate) =>
