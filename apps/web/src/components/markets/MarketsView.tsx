...
export const MarketsView: React.FC = () => {
  ...
  // Compute live total 24h market volume across all tracked tokens
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6">
      <div className="glass-panel rounded-2xl p-6 border border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-mono text-[10px] font-bold">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
              LIVE FEED
            </span>
          </div>
          <p className="text-sm text-slate-400 mt-1">
            Real-time live prices, 24h volume, and automated security scoring across {defaultChainRegistry.getAllChains().length} supported networks
          </p>
        </div>
      ...
      {/* API Warning Notice if any error */}
      {marketsError && (
        <div className="flex items-center gap-2 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs">
          ...
          <span>Live pricing feed: {marketsError}. Serving cached market prices.</span>
        </div>
      )}
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800/50 font-mono">
              {filteredTokens.map((t) => {
                ...
                const live = marketData[tokenKey];
